...
目标 cube 节点须已部署运行时并使用 TS4；省略 `--cube-node` 时选择带 `agc.cloud.tencent.com/cube-ready=true` 标签的可调度 Ready TS4 节点。`--runc-node` 可指定另一台物理节点；未找到时自动复用 Cube 节点，以验证 cube/runc 两个 handler。AWV CSI 用例要求目标节点已部署对应 CSI 插件和 `awv-btrfs` StorageClass。

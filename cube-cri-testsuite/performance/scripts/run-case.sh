...
case "$test_name" in
  sysbench-cpu)
    output="$(sysbench cpu --threads=1 --time=10 --cpu-max-prime=20000 run)"
  ...
  stream-triad)
    ...
    ;;
  lmbench-lat-mem-rd)
    output="$(/opt/lmbench/bin/*/lat_mem_rd -N 3 -W 1 -s 134217728 -e 134217728 -c 1000000 128M 64 2>&1)"
    value="$(awk 'NF >= 2 && $1 ~ /^[0-9]+(\.[0-9]+)?$/ {last=$NF} END {if (last == "") exit 1; print last}' <<<"$output")"
    emit "$value" "ns" '{"tool":"LMbench","metric":"lat_mem_rd_128M_stride64","higher_is_better":false}'
    ;;
  lmbench-bw-mem)
    output="$(/opt/lmbench/bin/*/bw_mem 128M rd 2>&1)"
  ...
  lmbench-pagefault)
    ...
    dd if=/dev/zero of="$file" bs=1M count=64 conv=fsync status=none
  ...
  lmbench-mmap)
    ...
    dd if=/dev/zero of="$file" bs=1M count=64 conv=fsync status=none
    output="$(/opt/lmbench/bin/*/lat_mmap -N 1 -W 0 64M "$file" 2>&1)"
    value="$(last_number <<<"$output")"
    rm -f "$file"
  ...
  hackbench-process|hackbench-socket)
    ...
    output="$(hackbench "$flag" -l 1000 -g 10 2>&1)"
  ...
  iperf-tcp1|iperf-tcp4)
    ...
    output="$(iperf3 -c "$iperf_host" -p "$iperf_port" -t 30 -P "$streams" -J)"
  ...
  iperf-loopback1|iperf-loopback4)
    ...
    server_pid=$!
    sleep 1
    if ! output="$(iperf3 -c 127.0.0.1 -p "$local_port" -t 30 -P "$streams" -J)"; then

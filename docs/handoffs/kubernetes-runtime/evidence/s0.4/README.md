...
`e3205220` 增加 `Coordinator`，固定 Coordinator → Registry → Store 锁序：进行中的 duplicate 先完成，Release 在 Registry 锁内持久化并在解锁前删除 binding；确定未提交时保留 READY，重启仅发布 READY，replacement 复用同一路径。`ServeConn` 缺少 authorizer 时 fail closed，并以真实 `SO_PEERCRED` 验证 allow/deny，拒绝路径不调用 TAP opener 且携带 0 FD。组合测试及 20 轮 race 重放均通过。

## 第三轮独立审查与整改

第三轮结果仍为 `CHANGES_REQUIRED`：rename 已完成但父目录 fsync 失败属于 commit-unknown，旧实现仍保留 READY binding；原 failure 测试只覆盖调用 Store 前失败。

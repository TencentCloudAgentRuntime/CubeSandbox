...
S0.4 v1 契约已按首轮独立审查整改，当前等待复审。宿主 containerd 独占 CRI、OCI image/snapshot、Sandbox/Task 与 CNI 状态；Cubelet RuntimeResource 仅准备和释放节点资源；Guest Agent 只负责 VM 内容器执行。不得在复审 `APPROVE` 前把 S0.4 标为 `DONE`。
...
| 持久化 lease/tombstone 状态机、FD handoff v1、direct gRPC registration 与真实 import-graph 测试 | `ea192ecb` |
...
- FD 请求冻结五个 lease 身份字段和 versioned frame；校验与 duplicate 同一临界区，Release/replace 先 invalidate；错误响应强制 0 FD，成功恰好 1 FD；明确 retry ownership、deadline 与 `SO_PEERCRED`。
...
- 测试覆盖并发 replacement/duplicate 原子性、stale/released lease、重试 fresh FD、非法/部分 frame、peer credential、错误无 FD，以及状态机 restart/tombstone/key/lease 组合。
...
同一独立 subagent 必须复查 `ea192ecb`、本证据、接口文档与测试，并明确返回 `APPROVE`；否则继续整改和复审，不进入 S1。

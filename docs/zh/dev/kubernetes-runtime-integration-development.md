...
| S3.4 资源控制 | `IN_PROGRESS` | Codex | S3.4a、S3.4b 均由同一 reviewer 批准为 `DONE`；Guest per-container cgroup 的 create/update、事务、压力、失败与兼容矩阵已关闭 | `inv-8853vxgxh1` 数值矩阵；`inv-9855pngtst` 压力/OOM/PendingCreate；`inv-08564809p0` capability fail-closed；`inv-68569j047d` legacy；`inv-v856bt07f9` Kubernetes resize；同一 reviewer `APPROVE S3.4b DONE` | 执行 S3.4c.1 Host Pod VM 包络设计冻结 |
...
| S3.4c.2 Host cgroup 生命周期与进程归属 | `IN_PROGRESS` | 为 managed Sandbox 创建唯一 Pod leaf并从首次分配起放置 Shim/VMM/virtiofs；legacy Task 不迁移；长期 scanner 覆盖 bundle/socket/leaf | classifier、path、gate、immutable/containment identity、pidfd、PREPARED→CONTAINERD_COMMITTED、scanner systemd supervision、response 后 failpoints、systemd 200 次竞态门禁、安全回收与双 owner 先 handoff通过；sibling cgroup breach 只杀原 server/PID 诱饵存活；创建失败、删除、重启零误杀零残留；同一 reviewer `APPROVE S3.4c.2` |

...
| S0.4 组件接口 | `VALIDATING` | Codex | RuntimeResource v1、持久化 lease 状态机、带 generation/lease/token 栅栏的 FD handoff、直接 gRPC 注册、Agent capability negotiation 已实现；首轮审查问题已整改 | 实现 `40f4389a`、`30bf3365`、`eb7aed1a`、`2e2612a4`、整改 `ea192ecb`；[验收证据](../../handoffs/kubernetes-runtime/evidence/s0.4/README.md) | 独立 subagent 审查至 `APPROVE` |

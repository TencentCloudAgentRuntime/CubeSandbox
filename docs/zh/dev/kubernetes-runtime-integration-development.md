...
| S3.4 资源控制 | `IN_PROGRESS` | Codex | S3.4a、S3.4b、S3.4c 均已 `DONE`；S3.4d 已启动 | `2a23aa3c` 关闭启动阶段 Guest OOM 与 inactive scope cleanup 竞态；六 Pod/九容器逐对象证据、Host CPU/memory/PIDs、Guest `OOMKilled/137`、survivor、containerd restart 和双 Worker exact zero 已闭环；同一 reviewer 明确 `APPROVE S3.4c DONE`；[S3.4c 证据](../../handoffs/kubernetes-runtime/evidence/s3.4/s3.4c.4-execution-summary.md) | 执行 S3.4d，关闭后立即运行 Kubernetes E2E/Conformance |
...
| S3.4d 压力回归与支持矩阵 | `IN_PROGRESS` | 组合验证 Host/Guest 双层限制、QoS、更新、故障与非 cgroup 资源，并冻结首版范围 | 多容器 CPU/内存压力、定向 OOM、in-place update、swap/hugepage/PIDs、ephemeral-storage 责任边界均有通过或明确拒绝证据；lease/Pod/VM/cgroup 全量基线恢复；独立审计和同一 reviewer `APPROVE S3.4 DONE` | 优先复现 `K8S-OQ-021/022`，再执行双层资源、swap/hugepage/PIDs、ephemeral-storage 组合矩阵 |

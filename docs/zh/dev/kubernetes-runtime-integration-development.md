...
| S3.4 资源控制 | `IN_PROGRESS` | Codex | S3.4a、S3.4b、S3.4c.1～S3.4c.3 已 `DONE`；S3.4c.4 候选进入 reviewer 门禁 | `90026f59` 修复 Guest OOM 通知竞态；六 Pod QoS/多容器三层数值、Host CPU/memory/PIDs、Guest `OOMKilled/137`、survivor、resize、containerd restart 和双 Worker exact zero 全通过；[候选证据](../../handoffs/kubernetes-runtime/evidence/s3.4/s3.4c.4-execution-summary.md) | 取得 `APPROVE S3.4c DONE` 后执行 S3.4d，再运行 Kubernetes E2E/Conformance |
...
| S3.4c.4 云端压力、故障与兼容审计 | `VALIDATING` | 验证 Host 上限、隔离、OOM、更新、清理和旧路径兼容 | `90026f59` 修复 cgroup v2 OOM/exit 通知竞态；`inv-a86gc90t3r` 等完成 QoS/多容器三层数值、Host CPU/memory/PIDs、Guest OOM、survivor、resize 和 containerd restart；最终 `inv-386h170wa5` 为 `OOMKilled/137`，`inv-086h2egmq0`/`inv-086h2f0c1g` 双节点 exact zero，`inv-386h37gkc2` 集群健康；等待同一 reviewer `APPROVE S3.4c DONE`；[候选证据](../../handoffs/kubernetes-runtime/evidence/s3.4/s3.4c.4-execution-summary.md) |

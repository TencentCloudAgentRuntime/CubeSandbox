...
| S3.4 资源控制 | `IN_PROGRESS` | Codex | S3.4a、S3.4b、S3.4c.1～S3.4c.3 已 `DONE`；当前执行 S3.4c.4 | S3.4c.3 `be91f9e0`、固定 builder 157/157、双节点 gate 200、真实 V1/resize/controller crash/V12 conflict/PIDs 零写入/exact cleanup 全通过并获同一 reviewer `APPROVE S3.4c.3`；[终验证据](../../handoffs/kubernetes-runtime/evidence/s3.4/s3.4c.3-execution-summary.md) | 完成 S3.4c.4 压力/兼容矩阵，再运行 Kubernetes E2E/Conformance |
...
| S3.4c Host Pod VM 资源包络 | `IN_PROGRESS` | 在 Host cgroup 对一个 Pod VM 的总 CPU/内存/进程资源实施静态 VM 容量 ceiling，并让 kubelet 父层和 Guest 容器层保持各自动态更新职责 | Host 预算算法有固定测试向量；VM/VMM/virtiofs/辅助进程归入唯一 Pod leaf；压力下祖先/leaf 交集生效且不影响其他 Pod；更新、OOM、删除和失败创建均收敛；同一 reviewer `APPROVE S3.4c DONE` | S3.4c.1～c.3 已 `DONE`；当前执行 S3.4c.4 云端压力、故障与兼容审计 |
...
| S3.4c.4 云端压力、故障与兼容审计 | `IN_PROGRESS` | 验证 Host 上限、隔离、OOM、更新、清理和旧路径兼容 | 执行 BestEffort/Burstable/Guaranteed、多容器/init/sidecar、CPU/内存/PIDs 压力、survivor、故障恢复与 exact baseline 矩阵；固定 Kubernetes v1.36.4 runner；同一 reviewer `APPROVE S3.4c DONE` |

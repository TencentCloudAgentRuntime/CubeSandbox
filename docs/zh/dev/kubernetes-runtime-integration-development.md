...
| S3.3 SecurityContext | `IN_PROGRESS` | Codex | S3.3a 已冻结六项安全字段并定位 NNP 缺口；S3.3b 已固化身份与组；S3.3c 已固化 capability 五集合、非法名称 fail-closed 和 rootfs 只读/可写语义；S3.3d 已修复 NNP 静默降级并固化 RuntimeDefault seccomp | S3.3d 实现 `5a851245`；最终构建 `inv-b8497x0qge`；部署 `inv-6849ef00t9`；终验 `inv-b849phgapi`；独立审计 `inv-3849tjgnmx`；[验收证据](../../handoffs/kubernetes-runtime/evidence/s3.3/README.md)；同一 reviewer `APPROVE S3.3d DONE` | S3.3e privileged 双门禁 |
...
| S3.3e privileged 双门禁 | `IN_PROGRESS` | 实现节点开关与 Pod 请求双门禁，只在 Guest 内提权 | 开关关闭时 privileged 请求明确失败；开关开启且 Pod 请求时才生效；普通 Pod 不提权；不自动透传 Host device/path；清理闭环 | 先冻结 Kubernetes privileged 的 OCI 输入、当前 Guest 行为及节点开关归属，再实现双门禁和正反例 |

...
| S3.4c Host Pod VM 资源包络 | `IN_PROGRESS` | 在 Host cgroup 对一个 Pod VM 的总 CPU/内存/进程资源实施上限，并正确处理 init、restartable sidecar、app 与动态更新 | Host 预算算法有固定测试向量；VM/VMM/virtiofs/辅助进程归入唯一 Pod cgroup；压力下 Host 上限生效且不影响其他 Pod；更新、OOM、删除和失败创建均收敛；同一 reviewer `APPROVE` | 当前执行 S3.4c.1 设计冻结；依赖 S3.4a Host 拓扑和已关闭的 S3.4b Guest 语义 |
...
| 实现单元 | 状态 | 目标 | 验收标准/下一步 |
|---|---|---|---|
| S3.4c.1 包络输入与生命周期设计 | `IN_PROGRESS` | 冻结 Pod 预算输入、init/restartable sidecar/app 算法、Host cgroup owner/path、进程归属、创建/更新/删除/恢复状态机 | 固定计算向量覆盖 QoS、多容器、init/sidecar、缺失 request/limit 和 PIDs；明确调用顺序、幂等键、rollback/fail-stop、containerd 重启恢复和与 Guest transaction 的边界；同一 reviewer `APPROVE S3.4c DESIGN` |
| S3.4c.2 Host cgroup 生命周期与进程归属 | `NOT_STARTED` | 创建唯一 Pod cgroup 并把 Shim、VMM、virtiofs 及所有辅助线程/进程放入 | 单测和特权集成测试证明唯一 owner、无逃逸、创建失败/删除/重启零残留；同一 reviewer `APPROVE S3.4c.2` |
| S3.4c.3 预算应用与动态更新事务 | `NOT_STARTED` | 应用 CPU/内存/PIDs 包络，并在容器变更和 in-place resize 时重算 | 固定向量与真实 controller readback 一致；Guest/Host 更新失败可重试且不交叉超卖；并发与重启测试通过；同一 reviewer `APPROVE S3.4c.3` |

...
| S5.4 启动架构、性能与稳定性 | `IN_PROGRESS` | Codex | S5.4a/b 已完成：冻结 CubeShim/Cubelet/worker 边界并将 VMM、vCPU、virtiofs、Guest memory 拆到每 Pod 一个 `cube-vmm-worker`；双节点普通/多容器、containerd/RuntimeResource 重启、worker/Shim kill、创建取消、embedded 回滚和 exact-zero 均通过 | 实现 `7a4d5554 → 26d3ca74 → c949d6d4 → 94c01e21`；release `inv-9895b6gsru`；[S5.4b 证据](../../handoffs/kubernetes-runtime/evidence/s5.4/s5.4b-worker-split.md)；同一 reviewer `PASS`，P0/P1/P2=0 | S5.4c 消除正常路径重复 `systemctl show`，完成 50 次串行/10 并发性能入口门禁 |

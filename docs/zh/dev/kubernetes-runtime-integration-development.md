...
| S3.4c.2 Host cgroup 生命周期与进程归属 | `VALIDATING` | 为 managed Sandbox 创建唯一 Pod leaf并从首次分配起放置 Shim/VMM/virtiofs；legacy Task 不迁移；长期 scanner 覆盖 bundle/socket/leaf | `fe2f47cb` 完成 classifier/path/gate/identity/pidfd、external lifecycle、Create/Delete failure barrier 与 epoch fencing，同一 reviewer P0/P1=0；`inv-085uf4g092` 50/50、`inv-885uk4gin4` systemd 200 gate、`inv-985umh0f8d` Pod/exact cleanup、`inv-v85umsgm7x` SIGKILL 后 un-drained 30.698 秒兜底通过；live delete 显式长 timeout 已功能通过，待干净重跑，然后完成 sibling lure、containerd restart、legacy 与最终 `APPROVE S3.4c.2` |

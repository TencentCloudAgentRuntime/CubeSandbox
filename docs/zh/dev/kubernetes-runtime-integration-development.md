...
| S3.3 SecurityContext | `IN_PROGRESS` | Codex | S3.3a 已冻结六项安全字段并定位 NNP 缺口；S3.3b 已固化身份与组；S3.3c 已固化 capability 五集合、非法名称 fail-closed 和 rootfs 只读/可写语义 | S3.3c 实现 `c55b759e`；终验 `inv-6846wbgtiv`；独立审计 `inv-6847cm06pi`；legacy race 回归 `inv-084814gvgg`；[验收证据](../../handoffs/kubernetes-runtime/evidence/s3.3/README.md)；同一 reviewer `APPROVE S3.3c DONE` | S3.3d NNP 与 seccomp |
...
| S3.3d NNP 与 seccomp | `IN_PROGRESS` | 去除 NNP 静默降级并固化 RuntimeDefault seccomp | host NNP=true 时 Guest `NoNewPrivs=1`；false/true 正反用例和 RuntimeDefault 过滤行为通过；关闭 `K8S-OQ-013`；清理闭环 | 修改前先冻结 Shim/Agent protobuf 兼容契约，再从 S3.3a 的 host/Guest 差异建立正反基线 |

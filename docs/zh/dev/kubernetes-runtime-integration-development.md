...
| S3.4 资源控制 | `IN_PROGRESS` | Codex | S3.4a、S3.4b 和 S3.4c.1 已 `DONE`；S3.4c.2 实现完成并进入云端验收 | S3.4b 证据保持；S3.4c.2 `fe2f47cb`、`inv-085uf4g092` 50/50、`inv-885uk4gin4` systemd gate 200、`inv-985umh0f8d` RuntimeClass Pod、`inv-v85umsgm7x` SIGKILL/30 秒兜底 | 关闭 S3.4c.2 剩余漂移、重启与 legacy 验收 |
...
| S3.4c.2 Host cgroup 生命周期与进程归属 | `VALIDATING` | 为 managed Sandbox 创建唯一 Pod leaf并从首次分配起放置 Shim/VMM/virtiofs；legacy Task 不迁移；长期 scanner 覆盖 bundle/socket/leaf | `fe2f47cb` 完成 classifier/path/gate/identity/pidfd、external lifecycle、Create/Delete failure barrier 与 epoch fencing；`ea441201`/`2269a3b3` 补齐并发目录、VMM 日志父目录和 sandboxer workload DNS 继承。原 50/50、systemd 200 gate、Pod/exact cleanup、SIGKILL/30 秒兜底保持通过；S0 双工作节点额外以 `inv-8863xngiss` 完成 8 个 Cube VM 的 64 次全互访、30 次跨节点、24 次稳定 DNS 与 24 次 ClusterIP，`inv-38640k0ru9`/`inv-88640n0hs5` 确认实际 `io.containerd.cube.rs` 和 3/5 VM/lease 一致，[证据](../../handoffs/kubernetes-runtime/evidence/s3.4/s0-cube-crossnode-workloads.md)。继续干净重跑 live delete，然后完成 sibling lure、containerd restart、legacy 与最终 `APPROVE S3.4c.2` |
| S3.4c.3 RuntimeClass overhead 与静态 leaf ceiling | `NOT_STARTED` | 消费标准 cgroup_parent/overhead，应用静态 VM CPU/memory/PIDs ceiling；kubelet parent 与 Guest Task Update 各自负责动态 resize | 固定向量与真实 controller readback 一致；节点 minimum 低值拒绝/高值接受；WAL 在每步 write 前 durable INTENT，server/scanner owner epoch、revoke 与 operation lock 保证唯一 writer，全部恢复决策/failpoint 可按 readback执行，反向 rollback/degraded replay、parent 只读与 leaf 静态验证通过；in-place resize 三层闭环且无需 containerd patch；同一 reviewer `APPROVE S3.4c.3` |
| S3.4c.4 云端压力、故障与兼容审计 | `NOT_STARTED` | 验证 Host 上限、隔离、OOM、更新、清理和旧路径兼容 | 多 Pod/多容器压力不越界且互不影响；失败矩阵、survivor、exact baseline、固定 SHA 和独立审计闭环；同一 reviewer `APPROVE S3.4c DONE` |

...
| S3.4b.3 degraded/PendingCreate | `IN_PROGRESS` | rollback failure fail-stop/undo replay、create 显式 owner 与可重试 cleanup | 完成状态机/失败注入单测和同一 reviewer 批准后进入 S3.4b.4 |

...
负载生成器调度到专用节点上。S4 使用 4 个实例，每个负责 5,000 Pod；预演阶段可按规模减少实例数。协调器在所有 Watch 建立后发布统一开始信号。每个生成器使用独立 Namespace 集合和 API 客户端，并保存逐请求结果。聚合目标速率按 Pod 分片比例分配给各生成器。不要在本地提交工作负载，本地与集群连接的网络链路请求延迟过高，会限制提交速率。

正式轮次采用开放负载：

- 聚合目标速率：`有效测试节点数 N × 20 Pod Create/s`。`N` 只统计本轮实际承载目标 Pod 的节点，不包含负载生成器、备用节点和被隔离节点。
- 目标提交窗口：`目标 Pod 数 ÷ (N × 20)`。例如 S1～S3 每节点 150 Pod 时约为 7.5 秒；S4 若使用 149 个有效节点，20,000 Pod 的目标提交窗口约为 6.7 秒。
- 聚合最大在途 Create 请求：2,000。
- 客户端不得因默认 client-go QPS 限流而低于目标速率。
- 到达时间、实际发送时间和响应时间分别记录；落后目标速率也必须如实计入 `T_submit`。

该定义中的“2 万并发”指一个批次中存在 20,000 个目标 Pod，并按 `N × 20 Pod/s` 提交，不表示建立 20,000 条同时进行的 HTTP 连接。若业务要求瞬时释放 20,000 个请求，另执行最大突发场景并单独报告。
...
| 阶段 | 计划节点范围 | Pod 数 | 目标 Create QPS | 目标提交窗口 | 目的 |
|---|---:|---:|---:|---:|---|
| S0 | 1 节点 | 1 | `1 × 20 = 20` | 约 0.05 秒 | 完整功能和清理检查 |
| S1 | 10 节点 | 1,500 | `10 × 20 = 200` | 约 7.5 秒 | 预计每节点 150 Pod，并发及业务探针检查 |
| S2 | 50 节点 | 7,500 | `50 × 20 = 1,000` | 约 7.5 秒 | 控制面、CNI、DNS、监控预演 |
| S3 | 100 节点 | 15,000 | `100 × 20 = 2,000` | 约 7.5 秒 | 半规模容量和提交速率验证 |
| S4 | `N` 个有效节点 | 20,000 | `N × 20` | `20,000 ÷ (N × 20)` 秒 | 正式测试 |

表中 S0～S3 的 QPS 是按计划节点数计算的示例，不是独立固定值。实际执行时必须重新统计本轮有效测试节点数 `N`，并以 `N × 20 Pod/s` 为唯一目标提交速率；实际节点数与计划不一致时，不得沿用表中的示例 QPS。
...
3. 协调器发布开始信号，4 个生成器按 `有效测试节点数 N × 20 Pod/s` 的聚合速率提交。
...
- [ ] 已按本轮有效测试节点数 `N` 将 20,000 Pod 的目标提交速率冻结为 `N × 20 Pod/s`，并冻结正式镜像 digest、冷态口径和 RuntimeTemplate 制品。

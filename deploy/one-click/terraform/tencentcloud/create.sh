...
setup_env() {
	...
	CUBE_IMAGE_TAG="${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.7.1}"
...
tcr_build_and_push() {
	...
	echo -e "  ${GREEN}✓ Image tag to build & push: ${CUBE_IMAGE_TAG:-v0.7.1}${NC}"
	echo ""
...
prompt_deployment_env() {
	...
	select_env TENCENTCLOUD_CUBE_IMAGE_TAG "Cube component image tag" "v0.7.1" "dev"
...
TENCENTCLOUD_CUBE_IMAGE_TAG='${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.7.1}'
...
write_resolved_tfvars_file() {
	...
	jq -n \
		...
		--arg image_tag "${TF_VAR_image_tag:-${CUBE_IMAGE_TAG:-${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.7.1}}}" \

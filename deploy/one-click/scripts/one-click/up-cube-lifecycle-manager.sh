...
# to both registries under the same :v0.7.1 tag; either default resolves
# to whatever the operator most recently published.
CUBE_LCM_IMAGE_INT_DEFAULT="cube-sandbox-int.tencentcloudcr.com/cube-sandbox/cube-lifecycle-manager:v0.7.1"
CUBE_LCM_IMAGE_CN_DEFAULT="cube-sandbox-cn.tencentcloudcr.com/cube-sandbox/cube-lifecycle-manager:v0.7.1"
...
render_template_atomic \
  ...
  -e "s#__CUBE_LCM_REDIS_MASTER_NAME__#$(escape_sed "${CUBE_LCM_REDIS_MASTER_NAME}" '#')#g" \
  -e "s#__CUBE_LCM_REDIS_SENTINEL_NODES__#$(escape_sed "${CUBE_LCM_REDIS_SENTINEL_NODES}" '#')#g" \
  -e "s#__CUBE_LCM_REDIS_SENTINEL_PASSWORD__#$(escape_sed "${CUBE_LCM_REDIS_SENTINEL_PASSWORD}" '#')#g" \

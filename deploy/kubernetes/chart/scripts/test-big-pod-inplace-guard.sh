...
grep -Eq 'kill -STOP[[:space:]]+"\$\$"' "${entrypoint}"
grep -q 'reuse_host_process' "${entrypoint}"
grep -q 'network-agent/bin/network-agent' "${entrypoint}"
grep -q 'REUSED_HOST_PID' "${entrypoint}"
grep -q 'CUBE_COMPONENT_FINGERPRINT' "${entrypoint}"

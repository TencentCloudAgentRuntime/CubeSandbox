...
> This host-network upgrade procedure requires `cubeNode.hostNetwork=true`; the default remains Pod networking.
> Changing modes replaces the old Pod. Migrate rules that depend on its CNI IP and validate one canary node first.

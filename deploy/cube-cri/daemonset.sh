...
node=${NODE:-}
mode=runtime
while (($#)); do case "$1" in --node) node=$2; shift 2;; --prepare-only) mode=prepare; shift;; *) echo "unknown argument: $1" >&2; exit 2;; esac; done
...
python3 - "$name" "$ns" "$node" "$image" "${CUBE_CRI_IMAGE_PULL_SECRET:-}" "$mode" > "$out/$name.json" <<'PY'
import json,sys
name,ns,node,image,secret,mode=sys.argv[1:]
...
pod={
    ...
    "containers":[{"name":"installer","image":image,"imagePullPolicy":"IfNotPresent",
        ...
        "env":[{"name":"POD_UID","valueFrom":{"fieldRef":{"fieldPath":"metadata.uid"}}},{"name":"CUBE_CRI_MODE","value":mode}],
...
if ! k -n "$ns" rollout status "daemonset/$name" --timeout=1800s; then

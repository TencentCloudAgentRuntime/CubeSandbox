...
# 或先构建运行时，再显式构建并推送安装镜像，最后部署。
# 默认推送到 ccr.ccs.tencentyun.com/journeyyou/cube-cri-installer。
task build
task package:image

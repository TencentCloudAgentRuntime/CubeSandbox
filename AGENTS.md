...
- 产生较复杂改动时, 应使用 subagent 从功能正确性、性能隐患、存量兼容性和可观测盲区几个角度进行code review，必须解决阻塞性问题后才能结束开发任务. 

- 在停止工作前, 应该回顾本次开发引入的改动, 识别哪些是调试期间所做的 workaround, 分析并验证此workaround是否是非必要的临时更改. 若是, 应该收敛改动并且到集群做复测.

- 当节点内出现 Cube runtime 之外的观测盲区和性能瓶颈时, 可利用 TKE_KUBERNETES_REPO 和 TKE_CONTAINERD_REPO 源码补充观测数据或A/B验证.

...
//
// This reconciler ONLY handles BUILT jobs (artifact ready, distribution
// pending). PENDING/RUNNING jobs are CubeTemplateCenter's responsibility —
// TC knows whether it crashed or was restarted, and sweeps its own
// in-progress builds with tier-specific timeouts (PENDING: 10min, PULLING:
// 30min, other phases: 1h). Master must NOT sweep those rows, because:
//   - It violates the build/distribution boundary (Master manages
//     distribution, TC manages builds).
//   - It produces inconsistent error messages (TC knows the actual failure
//     reason; Master can only guess).
//   - It requires keeping timeout thresholds in sync across two codebases,
//     which is error-prone.
//   - If TC is temporarily network-partitioned (not crashed), Master marking
//     the job FAILED would prevent TC from resuming the build when it
//     recovers.
		...
		if err != nil {
			...
			msg := fmt.Sprintf(
				"build finished but the post-build step was lost (likely a CubeMaster restart) "+
					"and cannot be replayed: %v. retry the build", err)
...
// failStaleRunningJobs was removed: PENDING/RUNNING jobs are CubeTemplateCenter's
// responsibility, not Master's. TC sweeps its own in-progress builds with
// tier-specific timeouts (PENDING: 10min, PULLING: 30min, other phases: 1h)
// in CubeTemplateCenter/pkg/reconcile/reconciler.go. Master must NOT sweep
// those rows, because:
//   - It violates the build/distribution boundary (Master manages
//     distribution, TC manages builds).
//   - It produces inconsistent error messages (TC knows the actual failure

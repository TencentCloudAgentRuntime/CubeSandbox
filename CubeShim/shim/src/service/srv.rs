...
use crate::service::{host_cgroup, runtime_resource};
...
impl Shim for Service {
    ...
    async fn start_shim(&mut self, opts: StartOpts) -> Result<String, Error> {
        ...
        crate::service::runner::durable_write_file(Path::new(ADDRESS_FILE), address.as_bytes())
    ...
    async fn delete_shim(&mut self) -> Result<api::DeleteResponse, Error> {
        let bundle = std::env::current_dir().map_err(|error| Error::IoError {
            context: "resolve delete bundle".to_string(),
            err: error,
        })?;
        let managed_lifecycle =
            host_cgroup::lifecycle_for_bundle(&self.ns, &self.id, &bundle).map_err(Error::Other)?;
        if let Some(lifecycle) = managed_lifecycle.as_ref() {
            lifecycle
                .request_cleanup_and_wait("containerd delete action")
                .await
                .map_err(Error::Other)?;
        } else {
            // Upgrade compatibility: an old shim has no immutable lifecycle
            // identity, so never signal the bare PID from shim.pid.  Persist
            // its RuntimeResource record for the long-running watchdog and
            // let containerd's graceful shutdown path own process exit.

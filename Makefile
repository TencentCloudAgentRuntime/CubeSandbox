...
cubelet: builder-image
	...
	$(MAKE) builder-run BUILDER_CMD='mkdir -p /workspace/_output/bin && cd /workspace && IN_CUBE_SANDBOX_BUILDER=1 make cubecow-sdk && cd /workspace/CubeNet/cubevs && make gen && cd /workspace/Cubelet && go mod download && make proto && make build && go build -o /workspace/_output/bin/cube-volume-block-device ./cmd/cube-volume-block-device && go build -o /workspace/_output/bin/cube-volume-erox-workspace ./cmd/cube-volume-erox-workspace && cp build/cubelet build/cubecli /workspace/_output/bin/'

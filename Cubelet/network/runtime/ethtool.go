...
// ifreqName copies an interface name into a NUL-terminated IFNAMSIZ buffer.
...
// ioctl sends an SIOCETHTOOL request whose payload is pointed to by ifreq.Data.
...
func featureIndex(fd int, iface, name string) (uint32, uint32, error) {
	...
	for i := uint32(0); i < n; i++ {
		...
		// The kernel returns fixed-width NUL-padded feature names. Trim at the
		// first NUL before comparing to the requested feature string.
...
// enableTXTCPMangleIDSegmentation enables an optional kernel offload feature for
// TAP devices. Failure is logged but non-fatal because older kernels or drivers
// may not expose the feature.
func enableTXTCPMangleIDSegmentation(iface string) {
	logger := CubeLog.WithContext(context.Background())
	logger.Infof("network runtime ethtool enable %s begin: iface=%s", featNameTxTCPMangleIDSegmentation, iface)

	fd, err := unix.Socket(unix.AF_INET, unix.SOCK_DGRAM, 0)
	if err != nil {
		logger.Warnf("network runtime ethtool enable %s open socket failed: iface=%s err=%v", featNameTxTCPMangleIDSegmentation, iface, err)
		return
	}
	defer unix.Close(fd)
	...
	if err != nil {
		logger.Warnf("network runtime ethtool enable %s lookup feature failed: iface=%s err=%v", featNameTxTCPMangleIDSegmentation, iface, err)
		return
	}

	err = setFeature(fd, iface, idx, total, true)
	if err != nil {
		logger.Warnf("network runtime ethtool enable %s set feature failed: iface=%s idx=%d total=%d err=%v", featNameTxTCPMangleIDSegmentation, iface, idx, total, err)
		return
	}

	logger.Infof("network runtime ethtool enable %s done: iface=%s idx=%d total=%d", featNameTxTCPMangleIDSegmentation, iface, idx, total)
}
